...
namespace
{
	...
	void forEachSdpLine(std::string_view body, const std::function<void(std::string_view, size_t)>& fn)
	{
		size_t pos = 0;
		while (pos < body.size())
		{
	...
	int parsePayloadType(std::string_view s)
	{
		int v = 0;
		bool any = false;
		for (char c : s)
	...
	AudioPolicyResult applyAudioPolicy(std::string_view body, bool allowWideband)
	{
		...
		forEachSdpLine(body, [&](std::string_view line, size_t pos) {
			if (line.rfind("a=rtpmap:", 0) == 0)
			{
				std::string_view rest = line.substr(9);
				int pt = parsePayloadType(rest);
				size_t sp = rest.find(' ');
			...
			else if (!r.hasMLine && line.rfind("m=audio ", 0) == 0)
			{
				...
				while (i < fmts.size())
				{
					...
					if (i > s)
					{
						int pt = parsePayloadType(fmts.substr(s, i - s));
...
bool SipMessage::filterAudioCodecs(bool allowWideband)
{
	...
	while (pos < body.size())
	{
		...
		{
			size_t colon = line.find(':');
			int pt = parsePayloadType(line.substr(colon + 1));

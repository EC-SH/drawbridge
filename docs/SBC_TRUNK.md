...
**Status:** The raw-SIP-trunk (B2BUA over SIP) design in this document is **not yet built**. Note: the adjacent **call-control-API trunk anchor** *has* shipped and is hardware-verified (outbound trunk calls with confirmed teardown + inbound Mode 1 ring-all against a commercial softswitch's HTTPS API — see [ARCHITECTURE.md](ARCHITECTURE.md) §6); this doc covers the raw-SIP variant only. | **Scope:** Engineering design. | **Last updated:** 2026-06-15

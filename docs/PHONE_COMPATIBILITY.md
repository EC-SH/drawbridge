...
| Audio codec | **G.711** µ-law (PCMU, `0`) / a-law (PCMA, `8`) always; **G.722** (`9`) between two phones that both offer it (peer-to-peer legs only — the echo test, register beep and the PSTN anchor stay G.711). DTMF telephone-event (any payload number, `101` by convention) passes through — but note star codes listen to SIP INFO, not RFC 2833 |
...
> [!IMPORTANT]
> **Codec is still the most important setting, but it now fails loudly.** DRAWBRIDGE does
...
> G.711 enabled on every client so calls to the echo test, the register beep and the
> PSTN anchor — which stay pinned to `0 8 101` via `enforceG711()` — still work.

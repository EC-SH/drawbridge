...
| **P2** | **Conferencing (mixing)** | ~~True N-way mixing needs an RTP mixer (DSP + media bandwidth) the MCU does not have.~~ **Reconsidered — see #164.** The anchor boundary already speaks linear int16 PCM; the only missing operation is the summing junction, and a working scalar `MixBus` (master-clock tick + per-port state machine, self-test green) proves the budget exists at ≤8 narrowband legs. | ~~**XL (off-device)**~~ → **on-MCU, in progress** | ~~Only as a relay to an external mixer; on-MCU mixing is a non-goal (§6).~~ Now the **N-way Conference Mix Bus** epic (#164–#170). |
...
| **P0/P1** | **SIP digest auth** (`REGISTER`/`INVITE` challenge) — **REGISTER + INVITE challenge shipped** (`SipDigest`/`SipSecretStore`, open/learn/secure registrar modes; INVITE `407` via #125) | Stops extension spoofing (S-3) and BYE-based teardown (D-2) *even on a trusted link*. Read-only credential-lookup callback into `RequestsHandler` (no new lock). | **shipped** | [THREAT_MODEL.md](THREAT_MODEL.md) §7 P1; seam from PROVISIONING.md §7.3 shipped as `SipSecretStore` |
...
|----------|-----------------|
| ~~**On-MCU media mixing / conferencing**~~ **(no longer a non-goal — being built, #164)** | ~~Requires an RTP mixer (CPU + media bandwidth + buffers) the MCU cannot afford while running SIP + LVGL. Breaks the "server never touches RTP" invariant ([SCALING.md](SCALING.md) §1). Only viable as a relay to an *external* mixer.~~ **Superseded.** The WAN anchor already touches RTP for the trunk leg, and linear PCM is already the currency at that boundary — the summing junction is the only addition. The **N-way Conference Mix Bus** (#164) lands it on-MCU: int32-accumulate, subtract-self, saturate once; one master-clock mix tick; a `Free→Active→Draining→Free` per-port state machine; confirmed-ISA S3 PIE kernels for when leg count climbs. Scalar reference compiled and self-test green. |
...
|-----|-----------|-----|-------------------|
| **P0** | **SIP digest auth** — REGISTER `401` + INVITE `407` shipped (RFC 2617 MD5, #125); RFC 8760 SHA-256 is the remaining target | Every commercial fabric challenges credentials — no auth, no trunk. Also closes the open registrar (already P0 in §3.3). | ~tens of KB; read-only credential callback, no new lock |

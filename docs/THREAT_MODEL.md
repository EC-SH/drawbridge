...
it. SIP **digest authentication** (RFC 2617, MD5 / `qop=auth`) challenges **REGISTER** — and,
as of #125, **INVITE** too: in `secure` mode an initial INVITE without a valid
`Proxy-Authorization` is challenged `407 Proxy Authentication Required` and verified against the
From-extension's HA1 (mid-dialog re-INVITEs and star codes are exempt). The registrar mode
becomes **runtime-selectable** (`open` / `learn` / `secure`);

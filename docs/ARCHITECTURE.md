...
### Inbound calls (Mode 1)
The anchor also accepts **inbound PSTN→extension calls**. When the upstream softswitch signals an incoming participant on the WebSocket event channel, `RequestsHandler::routeInboundAnchorCall()` rings the local extension bound to the trunk's source DN by sending a **delayed-offer INVITE** (no SDP) toward the registered handset; the handset's 200 OK carries the SDP answer, the anchor leg is answered at that point ("answer on handset 200"), and the `MediaBridge` is started against the handset's advertised RTP endpoint. This path is implemented end-to-end but **not yet hardware-verified**.

...
### D. Soak Telemetry on `/api/status`
For long-running soak tests and fleet health, `GET /api/status` carries a `telemetry` object alongside the registrar snapshot. It is assembled from the same lock-free path (anchor/media/pool counters come from the handler; heap/PSRAM/reset-reason are read from the system) so it adds no new locking on the RT path. Fields include the anchor connection state and estimated TLS socket count (`anchorConnected`, `mediaActive`, `tlsSocketsEst`), TLS full-vs-resumed handshake counts (`tlsFullHandshakes`, `tlsResumedHandshakes` — the resumption win is visible here), RTP `PlayoutBuffer` `playoutUnderruns`/`playoutOverruns`, client/session pool occupancy (`clientPool`/`sessionPool` `{used,cap}`), heap and PSRAM headroom (`freeHeap`, `minFreeHeap` low-water, `psramFree`, `psramTotal`), and the last-boot `resetReason` (`poweron`/`panic`/`brownout`/`task_wdt`/… — crash-relevant for soak). On the host build these emit `0`/`"host"` so the schema is stable everywhere. See [API.md](API.md) for the full schema.
...
### Media plane
* **Both-ways media cut-through (pre-warm + TLS resumption)**: both audio streams are primed ahead of the answer so the media path is warm when the leg connects. The downstream audio receiver (GET) task is spawned at the *Ringing* event (`startRxIfNeeded`) so its retry loop is already polling on a warm TLS connection; the upstream (POST) audio client uses a persistent, **resumable** TLS session so reconnecting injects audio without a fresh handshake. Because the S3 has no ECC/ECDSA hardware, the ECDHE handshake is software (~1 s); TLS session resumption is the workaround — a resumed POST connect drops from ~682 ms to ~116 ms, eliminating answer-time dead air. (One asymmetry: the upstream won't route a pre-Connect POST, so the outbound stream is primed via a one-shot at answer rather than at ring.)
...
### Inbound calls (Mode 1 — ring-all)
The anchor also accepts **inbound PSTN→extension calls**. When the upstream softswitch signals an incoming participant on the WebSocket event channel, `RequestsHandler::routeInboundAnchorCall()` **forks a delayed-offer INVITE (no SDP) to every registered extension** (ring-all); the first handset to answer with a 200 OK wins, its SDP answer is used to answer the anchor leg at that point ("answer on handset 200"), the losing forks are CANCELed, and the `MediaBridge` is started against the answering handset's advertised RTP endpoint. This path is **hardware-verified pre-alpha** — ring-all to every extension, two-way audio, and DTMF are confirmed on hardware; soak/stability hardening is in progress. (This cleared the project's capability gate: inbound PSTN was previously the last unverified leg.)
